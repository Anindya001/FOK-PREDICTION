...
class FractionalWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.sensitivity_study_tab: Optional[QWidget] = None
    ...
    def _build_ui(self) -> None:
        ...
        sens_form.setFieldGrowthPolicy(QFormLayout.AllNonFixedFieldsGrow)

        self.sensitivity_check = QCheckBox("Run sensitivity analysis during forecast")
        self.sensitivity_check.stateChanged.connect(self._apply_sensitivity_state)
        sens_form.addRow(self.sensitivity_check)

        self.sobol_horizons_edit = QLineEdit("200.0")
        self.sobol_horizons_edit.setPlaceholderText("e.g., 200, 400")
        self.sobol_horizons_edit.setClearButtonEnabled(True)
        self.sobol_horizons_edit.setToolTip("Comma-separated mission horizons for Sobol QoIs (positive numbers).")
        sens_form.addRow("Horizons", self.sobol_horizons_edit)
        ...
        self.thresholds_edit.setToolTip("Comma-separated failure ratios used for forecasts and Sobol QoIs.")
        sens_form.addRow("Failure thresholds", self.thresholds_edit)
        ...
        sens_form.addRow("Sobol samples", self.sobol_samples_spin)
        ...
        sens_form.addRow("Bootstrap draws", self.sobol_bootstrap_spin)
    ...
    def _build_sensitivity_tab(self) -> None:
        ...
        if MATPLOTLIB_AVAILABLE:
            self.sens_fig = Figure(figsize=(6, 4))
    ...
    def _build_forecast_tab(self) -> None:
        ...
        if MATPLOTLIB_AVAILABLE:
            self.forecast_fig = Figure(figsize=(10, 6))
    ...
    def _build_sensitivity_study_tab(self) -> None:
        if self.sensitivity_study_tab is not None:
            return
        study = QWidget()
        layout = QVBoxLayout(study)
        layout.setSpacing(10)
        ...
        prior_form.addRow("C₀ mean", self.sens_c0_mean)
        ...
        prior_form.addRow("C₀ log-sigma", self.sens_c0_sigma)
        ...
        prior_form.addRow("α mean", self.sens_alpha_mean)
        ...
        prior_form.addRow("α concentration", self.sens_alpha_conc)
        ...
        prior_form.addRow("f∞ mean", self.sens_finf_mean)
        ...
        prior_form.addRow("f∞ concentration", self.sens_finf_conc)

        layout.addWidget(prior_group)

        horizon_row = QHBoxLayout()
        horizon_row.addWidget(QLabel("Horizons:"))
        self.sens_horizons_edit = QLineEdit("200.0")
        self.sens_horizons_edit.setPlaceholderText("Comma-separated (e.g., 100, 200, 400)")
        self.sens_horizons_edit.setClearButtonEnabled(True)
        horizon_row.addWidget(self.sens_horizons_edit)
        threshold_label = QLabel("Failure thresholds:")
        horizon_row.addWidget(threshold_label)
        self.sens_thresholds_edit = QLineEdit("0.80, 0.70")
        self.sens_thresholds_edit.setPlaceholderText("0 < q < 1 (e.g., 0.85, 0.70)")
        self.sens_thresholds_edit.setClearButtonEnabled(True)
        horizon_row.addWidget(self.sens_thresholds_edit)
        layout.addLayout(horizon_row)

        sampling_row = QHBoxLayout()
        sampling_row.addWidget(QLabel("Sobol samples:"))
        self.sens_samples_spin = QSpinBox()
        self.sens_samples_spin.setRange(128, 32768)
        self.sens_samples_spin.setSingleStep(128)
        self.sens_samples_spin.setValue(2048)
        sampling_row.addWidget(self.sens_samples_spin)
        sampling_row.addWidget(QLabel("Bootstrap draws:"))
        self.sens_bootstrap_spin = QSpinBox()
        self.sens_bootstrap_spin.setRange(10, 5000)
        self.sens_bootstrap_spin.setSingleStep(10)
        self.sens_bootstrap_spin.setValue(200)
        sampling_row.addWidget(self.sens_bootstrap_spin)
        ...
        if MATPLOTLIB_AVAILABLE:
            self.sens_save_plot_btn = QPushButton("Save sensitivity plot…")
        ...
        if MATPLOTLIB_AVAILABLE:
            self.sens_study_fig = Figure(figsize=(7, 5))
            self.sens_study_fig.set_facecolor("white")
            self.sens_study_canvas = FigureCanvas(self.sens_study_fig)
            layout.addWidget(self.sens_study_canvas, stretch=1)
        else:  # pragma: no cover
            layout.addWidget(QLabel("Matplotlib not available."), stretch=1)

        self.sens_log = QPlainTextEdit()
        self.sens_log.setReadOnly(True)
        layout.addWidget(self.sens_log, stretch=1)

        self.sens_study_results: Dict[str, Dict[str, Any]] | None = None
        self.tabs.addTab(study, "Sensitivity study")
        self.sensitivity_study_tab = study
    ...
    def _mode_changed(self) -> None:
        ...
        if hasattr(self, "tabs") and self.tabs is not None:
            if self.current_mode == "sensitivity":
                if self.sensitivity_study_tab is None:
                    self._build_sensitivity_study_tab()
                self.tabs.setCurrentWidget(self.sensitivity_study_tab)
    ...
    def _apply_sensitivity_state(self) -> None:
        ...
        enabled = allow and getattr(self, "sensitivity_check", None) is not None and self.sensitivity_check.isChecked()
        horizon_widget = getattr(self, "sobol_horizons_edit", None)
        sample_widget = getattr(self, "sobol_samples_spin", None)
        bootstrap_widget = getattr(self, "sobol_bootstrap_spin", None)
        if horizon_widget is not None:
            horizon_widget.setEnabled(enabled)
        if sample_widget is not None:
            sample_widget.setEnabled(enabled)
        if bootstrap_widget is not None:
            bootstrap_widget.setEnabled(enabled)
        if hasattr(self, "thresholds_edit"):
            self.thresholds_edit.setEnabled(is_forecast)
    ...
    def _generate_baseline_comparison(self) -> None:
        ...
        try:
            ...
            for model_type, label, filename in models:
                ...
                ax.set_title(f"{label} forecast for {self._target_column}")
    ...
    def run_analysis(self) -> None:
        if self.current_mode == "sensitivity":
            self.tabs.setCurrentWidget(getattr(self, "sensitivity_study_tab", self.tabs.currentWidget()))
            QMessageBox.information(self, "Sensitivity study", "Use the Sensitivity Study tab to run the analysis.")
            return
    ...
    def _run_sensitivity_study(self) -> None:
        ...
        try:
            thresholds = self._parse_float_list(
                self.sens_thresholds_edit.text(),
                min_value=0.0,
                max_value=1.0,
                field_name="failure threshold",
            )
        ...
        params = {
            ...
            "samples": int(self.sens_samples_spin.value()),
            "bootstrap": int(self.sens_bootstrap_spin.value()),
        }
    ...
    def _on_sensitivity_study_success(self, payload: object) -> None:
        QApplication.restoreOverrideCursor()
        self._active_worker = None
        self.sens_run_button.setEnabled(True)
        ...
        if MATPLOTLIB_AVAILABLE and hasattr(self, "sens_save_plot_btn"):
            self.sens_save_plot_btn.setEnabled(bool(self.sens_study_results))
    ...
    def _update_forecast_plot(self) -> None:
        ...
        ax.set_xlabel(self._time_column or "Time")
        ax.set_ylabel(self._target_column or "Capacitance")
        ax.set_ylim(30.0, 100.0)
        ax.grid(True, alpha=0.25)
        if legend_entries:
